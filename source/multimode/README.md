...
## Dynamic Programming 
Suppose we are given a limit of total weight, and a set of items. Each item has its *weight (period)* and *profit (execution)*, and can be used unboundedly. What is the *largest* profit of items we can select? This problem can be solved efficiently by using **dynamic programming (DP)**.
