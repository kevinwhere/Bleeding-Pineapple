...
for m in $(eval echo "{$MStart..$MEnd..$MStep}")
do
if [ "$FPTDT" -eq "1" ]; then
	echo "" > tmpFPTDTm
fi
...
for u in $(eval echo "{$UStart..$UEnd..$UStep}")
	do	
		...
		do
			...
			python tg.py -o $TASKFILE -i $i -u $u -n $tasksinBkt -m $m
